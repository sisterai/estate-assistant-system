...
export function daviesBouldinIndex(data, labels) {
  ...
  const S = Object.fromEntries(
    Object.entries(byCluster).map(([c, idxs]) => {
      ...
      return [c,
        Math.sqrt(idxs.reduce((s,i) => s + euclidean(data[i],cent)**2,0) / idxs.length)
      ];
    })
  );
  const dbs = Object.keys(byCluster).map(c1 => {
    return Math.max(...Object.keys(byCluster).filter(c2=>c2!==c1).map(c2 =>
      (S[c1] + S[c2]) / euclidean(centroids[c1], centroids[c2])
    ));
  });
