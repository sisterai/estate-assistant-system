...
export async function trainPriceModel(rawRecords, {
  epochs=40, batchSize=32, lr=0.01, verbose=1
} = {}) {
  // 1) feature-engineer
  const data = computeFeatures(rawRecords);
  const X = data.map(d=>[d.pricePerSqft, d.age, d.bedroomBathRatio, d.priceAreaInteraction]);
...
export function predictPrice(model, newRecords) {
  const feats = computeFeatures(newRecords)
    .map(d=>[d.pricePerSqft, d.age, d.bedroomBathRatio, d.priceAreaInteraction]);
