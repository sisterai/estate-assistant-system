...
async function handleRun(body: any) {
  ...
  try {
    ...
    const durationMs = Date.now() - startedAt;
    return { status: 200, json: { runtime: 'default', goal, messages, durationMs } };
  } catch (e: any) {
    const durationMs = Date.now() - startedAt;
    return { status: 500, json: { error: e?.message || String(e), durationMs } };
  }
}
...
const server = http.createServer(async (req, res) => {
  ...
  if (req.method === 'GET' && url.pathname === '/config') {
    return sendJson(res, 200, { runtimes: ['default', 'langgraph', 'crewai'], defaultRounds: 5 });
  }
  if (req.method === 'GET' && url.pathname === '/run/stream') {
    ...
    try {
      if (runtime === 'langgraph') {
        ...
        const result = await runEstateWiseAgent({ input: goal, threadId: threadId || undefined });
      ...
      if (runtime === 'crewai') {
        send({ type: 'start', runtime, goal, rounds });
        const result = await runCrewAIGoal(goal);
        send({ type: 'message', message: { from: 'crewai', content: result?.output || JSON.stringify(result) } });
        send({ type: 'done' });
        clearInterval(heartbeat);
        res.end();
        return;
