...
export async function example5_OrchestratorIntegration() {
  ...
  // Execute
  const result = await orchestratorResult.pipeline.execute('Find properties near Duke University');
...
export async function example7_OrchestratorComposition() {
  ...
  const orchestratorResult = createOrchestratorPipeline(orchestrator, { rounds: 3 });
  ...
  console.log('Success:', result.success);
  console.log('Properties found:', (result.context?.state as any)?.propertyResults?.length);
  console.log('Financial analysis complete:', !!(result.context?.state as any)?.affordability);
}
...
export async function example8_LangGraphIntegration() {
  ...
  const pipeline = createPipeline<{ goal: string }, AgentPipelineState>()
    ...
    .addStage(
      createStage('process-results', async (context) => {
        const state = context.state as AgentPipelineState;
        console.log('LangGraph insights:', (state as any).langGraphAnalysis?.insights);
  ...
  console.log('Success:', result.success);
  console.log('LangGraph analysis:', (result.context?.state as any)?.langGraphAnalysis);
}
  ...
  const pipeline = createPipeline<{ goal: string }, AgentPipelineState>()
    ...
    .addStage(
      createStage('process-crew-output', async (context) => {
        const state = context.state as AgentPipelineState;
        console.log('Crew agents:', (state as any).crewAIResult?.agents?.length);
...
export async function example11_MultiRuntimePipeline() {
  ...
  console.log('Success:', result.success);
  console.log('Plan:', (result.context?.state as any)?.plan ? 'Generated' : 'None');
