...
describe('API module', () => {
  ...
  describe('renameConversation', () => {
    it('returns updated conversation on success', async () => {
      ...
      await expect(renameConversation(convId, title, token)).resolves.toEqual(updated);
      expect(fetch).toHaveBeenCalledWith(
  ...
  describe('sendMessage', () => {
    ...
    it('throws generic on missing error field', async () => {
      ...
      await expect(sendMessage({ message })).rejects.toThrow('Error sending message');
    });
  });
  ...
  describe('searchConversations', () => {
    ...
    it('throws server error message when not ok', async () => {
      ...
      await expect(searchConversations(query, token)).rejects.toThrow('Search failed');
    });
    ...
    it('throws generic when error field absent', async () => {
      ...
      await expect(searchConversations(query)).rejects.toThrow('Conversation search failed');
    });
  });
});
