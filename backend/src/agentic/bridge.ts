...
function collect(child: ReturnType<typeof spawn>, timeoutMs?: number): Promise<RunResult> {
  return new Promise((resolve) => {
    ...
    child.once('close', (code) => {
      ...
      resolve({ ok: code === 0, output: out.trim(), code, error: err.trim() || undefined });
    });
    child.once('error', (e) => {
      if (timer) clearTimeout(timer);
 ...
 */
export async function runAgentGoal(goal: string, opts: RunOptions = {}): Promise<RunResult> {
  ...
  const runFromDist = () => spawn(nodeCmd, ['dist/index.js', ...args], { cwd: agentDir, env });
  const runFromSrc = () => spawn(nodeCmd, ['node_modules/.bin/tsx', 'src/index.ts', ...args], { cwd: agentDir, env });
 ...
 */
export async function mcpListTools(opts: { parseJsonText?: boolean; preferBuild?: boolean; timeoutMs?: number } = {}) {
  const root = repoRootFromHere();
  ...
  const child = useBuild
    ...
    : spawn(nodeCmd, ['node_modules/.bin/tsx', 'src/client.ts', 'list'], { cwd: mcpDir, env });
...
export async function mcpCallTool(
  ...
  opts: { parseJsonText?: boolean; preferBuild?: boolean; timeoutMs?: number } = {},
) {
  const root = repoRootFromHere();
  const mcpDir = path.join(root, 'mcp');
  const env = { ...process.env };
  ...
  const child = useBuild
    ? spawn(nodeCmd, ['dist/client.js', 'call', name, argsJson, ...parseFlag], { cwd: mcpDir, env })
    : spawn(nodeCmd, ['node_modules/.bin/tsx', 'src/client.ts', 'call', name, argsJson, ...parseFlag], {
        cwd: mcpDir,
        env,
      });
...
if (require.main === module) {
  (async () => {
    ...
    const mode = argv[0];
    if (mode === 'run') {
      const runtime = (argv.find((a) => a.startsWith('--runtime='))?.split('=')[1] || 'orchestrator') as AgentRuntime;
      const goal = argv.slice(1).filter((a) => !a.startsWith('--runtime=')).join(' ');
      ...
      // eslint-disable-next-line no-console
      console.log('Usage: ts-node-dev src/agentic/bridge.ts run <goal> [--runtime=langgraph|crewai]');
      // eslint-disable-next-line no-console
      console.log('       ts-node-dev src/agentic/bridge.ts tools');
      // eslint-disable-next-line no-console
      console.log('       ts-node-dev src/agentic/bridge.ts call <tool> <json>');
    }
  })();
}
