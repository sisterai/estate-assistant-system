...
import { ValidationError, ValidationResult, convertZodErrors, validateObjectId } from './default.validation';
...
const timeWindowSchema = z.string().refine((val) => {
    ...
    message: "Window must be in HH:MM-HH:MM format with start time before end time"
});
...
const CreateCommuteProfileSchema = z.object({
    name: z.string()
        ...
        .trim(),
    destinations: z.array(CommuteDestinationSchema)
    ...
    maxMinutes: z.number()
        ...
        .max(300, "Max minutes cannot exceed 300")
        .optional(),
    combine: z.enum(['intersect', 'union'])
        .default('intersect')
        .optional()
});

// Update commute profile schema
const UpdateCommuteProfileSchema = z.object({
    name: z.string()
        .min(1, "Name cannot be empty")
        .max(100, "Name cannot exceed 100 characters")
        .trim()
