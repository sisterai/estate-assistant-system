import CommuteProfile, { ICommuteProfile } from "../models/CommuteProfile.model";
import { CreateCommuteProfileDto, UpdateCommuteProfileDto, CommuteProfileResponseDto } from "../dto/commute-profile.dto";
import { validateCreateCommuteProfile, validateUpdateCommuteProfile } from "../validations/commute-profile.validation";
...
export class CommuteProfileService {
     ...
     */
    async createProfile(userId: string, data: CreateCommuteProfileDto): Promise<CommuteProfileResponseDto> {
        // Validate input data
     ...
     */
    async getProfilesByUserId(userId: string): Promise<CommuteProfileResponseDto[]> {
        if (!validateObjectId(userId)) {
            throw AppError.badRequest("Invalid user ID format");
        }

        const profiles = await CommuteProfile.find({ userId: new mongoose.Types.ObjectId(userId) })
            .sort({ updatedAt: -1 });
     ...
     */
    async getProfileById(profileId: string, userId: string): Promise<CommuteProfileResponseDto> {
     ...
     */
    async updateProfile(profileId: string, userId: string, data: UpdateCommuteProfileDto): Promise<CommuteProfileResponseDto> {
        // Validate input data
