...
async function main() {
  ...
  if (resetMode) {
    console.warn(`NEO4J_RESET='${resetMode}' enabled. Clearing existing graph...`);
    if (resetMode === "all") {
  ...
  await runWrite(
    `CREATE INDEX IF NOT EXISTS FOR (z:Zip) ON (z.code)`
  );
  await runWrite(
  ...
  async function runWriteWithRetry<T = any>(cypher: string, params: Record<string, any>) {
    let attempt = 0;
    ...
    while (true) {
      try {
        ...
        console.warn(`Neo4j write failed (attempt ${attempt}/${maxRetries}). Retrying in ${delay}ms...`);
  ...
  let limit = rawLimit === "" || rawLimit.toLowerCase() === "all" || rawLimit === "0"
  ...
  console.log(`Starting Pinecone -> Neo4j ingest. Namespace='${namespace || ''}', Limit=${limit === Number.POSITIVE_INFINITY ? 'ALL' : limit}`);
  const nsIndex = namespace ? pineconeIndex.namespace(namespace) : pineconeIndex;

  // Checkpoint file support for resume
  const checkpointPath = process.env.INGEST_CHECKPOINT_FILE || path.join(process.cwd(), ".neo4j_ingest_checkpoint.json");
  function saveCheckpoint(nextToken: string | undefined, processed: number) {
    try {
      const data = { namespace, nextToken: nextToken || null, processed, pageSize: PAGE_SIZE, ts: new Date().toISOString() };
      fs.writeFileSync(checkpointPath, JSON.stringify(data, null, 2), { encoding: "utf8" });
    } catch {}
  }
  function loadCheckpoint(): { nextToken?: string; processed?: number; pageSize?: number } | null {
    try {
      if (!fs.existsSync(checkpointPath)) return null;
      const raw = JSON.parse(fs.readFileSync(checkpointPath, "utf8"));
      if (raw && raw.namespace === namespace) {
        return { nextToken: raw.nextToken || undefined, processed: Number(raw.processed || 0), pageSize: Number(raw.pageSize || PAGE_SIZE) };
      }
      return null;
    } catch {
      return null;
  ...
  const startTokenEnv = (process.env.PINECONE_START_TOKEN || "").trim() || undefined;
  ...
  const PAGE_SIZE = Math.max(1, Math.min(1000, Number(process.env.PINECONE_PAGE_SIZE || 100)));

  async function listPageOrSkip(token: string | undefined, pageLimit: number) {
    try {
      return await nsIndex.listPaginated({ limit: pageLimit, paginationToken: token });
    } catch (err) {
      // If token invalid and we have checkpoint, attempt skip-by-count fallback
      if (doResume && checkpoint && checkpoint.processed) {
        console.warn(`listPaginated failed (token may be invalid). Attempting skip-by-count fallback...`);
        let tempToken: string | undefined = undefined;
        // compute pages to skip based on processed count and page size
        const ps = checkpoint.pageSize && checkpoint.pageSize > 0 ? checkpoint.pageSize : PAGE_SIZE;
        const pagesToSkip = Math.floor((checkpoint.processed || 0) / ps);
        let resp = await nsIndex.listPaginated({ limit: ps, paginationToken: undefined });
        tempToken = resp.pagination?.next || undefined;
        for (let i = 1; i < pagesToSkip && tempToken; i++) {
          resp = await nsIndex.listPaginated({ limit: ps, paginationToken: tempToken });
          tempToken = resp.pagination?.next || undefined;
        }
        return await nsIndex.listPaginated({ limit: pageLimit, paginationToken: tempToken });
      }
      throw err;
    }
  }
  ...
  while (count < limit) {
    ...
    const pageLimit = Math.min(PAGE_SIZE, Number.isFinite(remaining) ? remaining : PAGE_SIZE);
    const listResp = await listPageOrSkip(paginationToken, pageLimit);
    console.log(`Fetched ID page: ${listResp.vectors?.length || 0} ids; nextToken=${listResp.pagination?.next ? 'yes' : 'no'}`);
    ...
    for (const id of ids) {
      ...
      if (typeof m.address === "string") {
        try { addr = JSON.parse(m.address); } catch { addr = null; }
      }
