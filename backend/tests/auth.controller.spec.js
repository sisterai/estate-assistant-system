...
describe('Auth Controller (JS spec)', () => {
  ...
  it('login → 400 if user not found or password mismatch', async () => {
    ...
    jest.clearAllMocks();
    findOneMock.mockResolvedValueOnce({
      _id: 'u2', username: 'alice', email: 'a@x.com', password: 'hashed:other'
    });
    bcrypt.compare.mockResolvedValueOnce(false);

    req = httpMocks.createRequest({ body: { email: 'a@x.com', password: 'pw' } });
    res = buildRes();
    await login(req, res);
    expect(res.status).toHaveBeenCalledWith(400);
    expect(res.json).toHaveBeenCalledWith({ error: 'Invalid credentials' });
  ...
  it('verifyEmail → 200 & echo email if found', async () => {
    ...
    expect(res.json).toHaveBeenCalledWith({ message: 'Email verified', email: 'ok@x' });
  });
});
