...
```mermaid
flowchart TD
  Alert[Alert/page] --> Triage[Triage severity]
  Triage --> Inspect[Inspect pods/events/logs]
  Inspect --> Mitigate[Rollback/scale/fix]
  Mitigate --> Verify[Verify health + metrics]
...
sequenceDiagram
  ...
  OnCall->>OnCall: initial triage
  alt unresolved > 15m
    OnCall->>Lead: escalate
    Lead->>Team: incident bridge
  end
  Team->>OnCall: mitigation support
  OnCall->>Team: resolved and verified
```
...
<AccordionGroup>
  <Accordion title="Service down">
    Check pod states, events, and image pull errors first. Validate resource
    availability and probe behavior.
  </Accordion>
  <Accordion title="High error rate">
    Check rollout history, recent changes, database connectivity, and upstream
    API failures. Roll back quickly when thresholds are exceeded.
  </Accordion>
  <Accordion title="Performance degradation">
    Check HPA, CPU/memory pressure, and query bottlenecks. Scale first, then
    optimize.
  </Accordion>
  <Accordion title="Canary failure">
    Verify canary pod health and metrics, then scale canary down and restore
    stable replicas.
  </Accordion>
</AccordionGroup>
