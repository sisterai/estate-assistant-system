...
export async function httpGet(path: string, opts: { headers?: Headers; baseUrl?: string } = {}) {
...
const responseCache = new LruCache<string, any>(config.cacheMax, config.cacheTtlMs);

/** Perform a GET request with in-memory LRU+TTL caching. */
export async function httpGetCached(path: string, opts: { headers?: Headers; baseUrl?: string } = {}) {
  const url = buildUrl(opts.baseUrl || config.apiBaseUrl, path);
...
export async function httpPost(path: string, body: unknown, opts: { headers?: Headers; baseUrl?: string } = {}) {
  const url = buildUrl(opts.baseUrl || config.apiBaseUrl, path);
  const headers: Headers = { 'Content-Type': 'application/json', ...(opts.headers || {}) };
  const res = await fetch(url, { method: 'POST', headers, body: JSON.stringify(body) });
  return handleJsonResponse(res as unknown as Response);
}

/** Perform a PUT request to the backend API. */
export async function httpPut(path: string, body: unknown, opts: { headers?: Headers; baseUrl?: string } = {}) {
  const url = buildUrl(opts.baseUrl || config.apiBaseUrl, path);
  const headers: Headers = { 'Content-Type': 'application/json', ...(opts.headers || {}) };
...
export async function httpDelete(path: string, opts: { headers?: Headers; baseUrl?: string } = {}) {
  const url = buildUrl(opts.baseUrl || config.apiBaseUrl, path);
