...
export const financeTools: ToolDef[] = [
  {
    ...
    handler: async (args: any) => {
      const { price, downPct = 20, apr = 6.5, years = 30, months = 12 } = args as any;
      const loan = price * (1 - downPct / 100);
      const r = apr / 100 / 12;
      const n = years * 12;
      const pmt = r ? (loan * r * Math.pow(1 + r, n)) / (Math.pow(1 + r, n) - 1) : loan / n;
      ...
      return { content: [{ type: 'text', text: JSON.stringify({ payment: pmt, schedule: rows }) }] };
    },
  },
  {
    name: 'finance.mortgage',
    description: 'Compute mortgage breakdown (principal+interest, taxes, insurance, HOA).',
    schema: {
      price: z.number(),
      downPct: z.number().default(20),
      apr: z.number().default(6.5),
    ...
    handler: async (args: any) => {
      const { price, downPct, apr, years, taxRatePct, insMonthly, hoaMonthly } = args as any;
      const loan = price * (1 - downPct / 100);
      const r = apr / 100 / 12;
      const n = years * 12;
      ...
      return { content: [{ type: 'text', text: JSON.stringify({ loan, pAndI, taxes, insMonthly, hoaMonthly, total }) }] };
    },
  },
  {
    ...
    handler: async (args: any) => {
      const { monthlyBudget, annualIncome, maxDtiPct = 36, downPct = 20, apr = 6.5, years = 30, taxRatePct = 1.0, insMonthly = 120, hoaMonthly = 0 } = args as any;
      const monthlyCap = monthlyBudget ?? (annualIncome ? (annualIncome / 12) * (maxDtiPct / 100) : null);
      ...
      const price = (monthlyCap - fixed) / (loanFactor * k + taxRatePct / 100 / 12);
      const loan = price * loanFactor;
      const pAndI = loan * k;
      const taxes = (price * (taxRatePct / 100)) / 12;
      const total = pAndI + taxes + fixed;
      return { content: [{ type: 'text', text: JSON.stringify({ monthlyCap, price, loan, pAndI, taxes, insMonthly, hoaMonthly, total }) }] };
    },
  },
  {
    name: 'finance.capRate',
    description: 'Compute cap rate given rent and expenses. Returns NOI and capRate.',
    schema: {
      price: z.number(),
      annualRent: z.number(),
    ...
    handler: async (args: any) => {
      const { price, annualRent, vacancyPct = 5, expensesAnnual = 0, taxRatePct, insuranceAnnual, hoaAnnual } = args as any;
      ...
      return { content: [{ type: 'text', text: JSON.stringify({ egi, opex, noi, capRate }) }] };
    },
  },
  {
    ...
    handler: async (args: any) => {
      const { monthlyRent, price, downPct = 20, apr = 6.5, years = 30, taxRatePct = 1.0, insMonthly = 120, hoaMonthly = 0 } = args as any;
      const loan = price * (1 - downPct / 100);
      const r = apr / 100 / 12;
      const n = years * 12;
      const pAndI = r ? (loan * (r * Math.pow(1 + r, n))) / (Math.pow(1 + r, n) - 1) : loan / n;
      ...
      return { content: [{ type: 'text', text: JSON.stringify({ ownMonthly, monthlyRent, difference: diff }) }] };
    },
  },
];
