...
export const authTools: ToolDef[] = [
  ...
  },
  {
    ...
    handler: async (args: any) => {
      const { username, email, password } = args as { username: string; email: string; password: string };
      const data = await httpPost('/api/auth/signup', { username, email, password });
      return { content: [{ type: 'text', text: JSON.stringify(data) }] };
    },
  },
  {
  ...
  },
  {
    ...
    handler: async (args: any) => {
      const { email, newPassword } = args as { email: string; newPassword: string };
      const data = await httpPost('/api/auth/reset-password', { email, newPassword });
      return { content: [{ type: 'text', text: JSON.stringify(data) }] };
    },
  },
];
