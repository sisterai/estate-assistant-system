...
export const systemTools: ToolDef[] = [
  {
    ...
    schema: {},
    handler: async () => {
      const safe = { apiBaseUrl: config.apiBaseUrl, frontendBaseUrl: config.frontendBaseUrl };
  ...
  },
  {
    ...
    schema: {},
    handler: async () => {
      return { content: [{ type: 'text', text: JSON.stringify({ now: new Date().toISOString() }) }] };
    },
  },
  {
    name: 'system.health',
    description: 'Check backend health by issuing a low-impact properties request.',
    ...
    handler: async (args: any) => {
      ...
      try {
        ...
        const ok = Array.isArray((data as any)?.results) || Array.isArray((data as any)?.properties);
        return { content: [{ type: 'text', text: JSON.stringify({ ok, sample: ok ? (data as any).results?.slice?.(0, 1) || (data as any).properties?.slice?.(0, 1) : null }) }] };
      } catch (e: any) {
  ...
  },
  {
    ...
    schema: {},
    handler: async () => {
      const tools = catalog.map((t) => ({ name: t.name, description: t.description }));
