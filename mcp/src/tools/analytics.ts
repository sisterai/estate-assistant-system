...
export const analyticsTools: ToolDef[] = [
  {
    ...
    handler: async (args: any) => {
      ...
      const nums = {
        price: items.map((p: any) => pickNum(p.price)).filter((n: number) => !isNaN(n)),
        sqft: items.map((p: any) => pickNum(p.livingArea)).filter((n: number) => !isNaN(n)),
        beds: items.map((p: any) => pickNum(p.bedrooms)).filter((n: number) => !isNaN(n)),
        baths: items.map((p: any) => pickNum(p.bathrooms)).filter((n: number) => !isNaN(n)),
      } as const;
      ...
      return { content: [{ type: 'text', text: JSON.stringify({ summary, sample: items.slice(0, 10) }) }] };
    },
  },
  {
    ...
    handler: async (args: any) => {
      ...
      const groups = Object.entries(byZip).map(([zip, prices]) => ({ zip, count: prices.length, medianPrice: median(prices) }));
      return { content: [{ type: 'text', text: JSON.stringify({ groups }) }] };
    },
  },
  {
    name: 'analytics.distributions',
    description: 'Compute quartiles and histogram buckets for price and living area.',
    schema: { q: z.string(), topK: z.number().optional(), buckets: z.number().optional() },
    handler: async (args: any) => {
      const { q, topK = 500, buckets = 10 } = args as { q: string; topK?: number; buckets?: number };
      const data = await httpGet(`/api/properties${qs({ q, topK })}`);
      const items = Array.isArray((data as any)?.results)
        ? (data as any).results
        : (data as any)?.properties || [];
      const pick = (k: string) => items
      ...
      const quantiles = (arr: number[]) => {
        if (!arr.length) return null;
        const a = [...arr].sort((a, b) => a - b);
        const q = (p: number) => a[Math.min(a.length - 1, Math.max(0, Math.floor((a.length - 1) * p)))];
        return { q0: a[0], q1: q(0.25), q2: q(0.5), q3: q(0.75), q4: a[a.length - 1] };
      };
      const histogram = (arr: number[]) => {
        ...
        return edges.slice(0, -1).map((start, i) => ({ start, end: edges[i + 1], count: counts[i] }));
      };
      return {
        content: [{
          type: 'text',
  ...
  },
  {
    ...
    description: 'Compute price-per-sqft distribution and summary for a search.',
    schema: { q: z.string(), topK: z.number().optional(), buckets: z.number().optional() },
    handler: async (args: any) => {
      const { q, topK = 300, buckets = 10 } = args as { q: string; topK?: number; buckets?: number };
      const data = await httpGet(`/api/properties${qs({ q, topK })}`);
      const items = Array.isArray((data as any)?.results)
        ? (data as any).results
        : (data as any)?.properties || [];
      const ppsf = (items as any[])
        .map((p: any) => ({ price: Number(p?.price ?? NaN), sqft: Number(p?.livingArea ?? NaN) }))
        .filter((x: { price: number; sqft: number }) => Number.isFinite(x.price) && Number.isFinite(x.sqft) && x.sqft > 0)
      ...
      return { content: [{ type: 'text', text: JSON.stringify({ count: ppsf.length, quantiles: quantiles(ppsf), histogram: histogram(ppsf) }) }] };
    },
  },
];
