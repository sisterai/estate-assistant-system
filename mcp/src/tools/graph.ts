...
export const graphTools: ToolDef[] = [
  {
    ...
    handler: async (args: any) => {
      ...
      const data = await httpGet(`/api/graph/similar/${Number(zpid)}${qs({ limit })}`);
  ...
  },
  {
    ...
    handler: async (args: any) => {
      ...
      const data = await httpGet(`/api/graph/neighborhood/${encodeURIComponent(name)}${qs({ limit })}`);
      return { content: [{ type: 'text', text: JSON.stringify(data) }] };
    },
  },
  {
  ...
  },
  {
    ...
    description: 'Explain relationships for up to 4 pairs derived from provided ZPIDs.',
    schema: { zpids: z.array(z.number()) },
    handler: async (args: any) => {
      const { zpids } = args as { zpids: number[] };
      const pairs: Array<[number, number]> = [];
      for (let i = 0; i < zpids.length - 1 && pairs.length < 4; i++) pairs.push([zpids[i], zpids[i + 1]]);
      const out: any[] = [];
      ...
      return { content: [{ type: 'text', text: JSON.stringify({ pairs: out }) }] };
    },
  },
  {
    name: 'graph.pathMatrix',
    description: 'Explain paths for adjacent pairs across provided ZPIDs (limited).',
    schema: { zpids: z.array(z.number()), limitPairs: z.number().optional() },
    handler: async (args: any) => {
      const { zpids, limitPairs = 6 } = args as { zpids: number[]; limitPairs?: number };
      const pairs: Array<[number, number]> = [];
      for (let i = 0; i < zpids.length - 1 && pairs.length < limitPairs; i++) pairs.push([zpids[i], zpids[i + 1]]);
      const out: any[] = [];
      for (const [from, to] of pairs) {
        try {
          const data = await httpGet(`/api/graph/explain${qs({ from, to })}`);
