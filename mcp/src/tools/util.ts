...
function pickJsonPath(obj: any, path: string): any {
  if (!path) return obj;
  return path.split('.').reduce((acc: any, key: string) => (acc && key in acc ? acc[key] : undefined), obj);
}
...
export const utilTools: ToolDef[] = [
  {
    ...
    handler: async (args: any) => {
      ...
      return { content: [{ type: 'text', text: JSON.stringify({ zpids: extractZpidsFromText(text) }) }] };
    },
  },
  {
  ...
  },
  {
    ...
    handler: async (args: any) => {
      const { text } = args as { text: string };
      const zpidMatches = Array.from(text.matchAll(/(\d+)_zpid|\bzpid[:#]?\s*(\d{5,})/gi))
        .map((m: RegExpMatchArray) => Number((m as any)[2] || (m as any)[1]?.replace('_zpid', '') || 0))
        .filter(Boolean);
      const zip = (text.match(/\b(\d{5})\b/) || [])[1] || null;
      const cityState = (text.match(/([A-Za-z][\w\s]+),\s*([A-Za-z]{2})/) || []) as any;
      ...
      if (priceRaw[1]) {
        ...
        const mult = priceRaw[2]?.toLowerCase() === 'm' ? 1_000_000 : priceRaw[2]?.toLowerCase() === 'k' ? 1_000 : 1;
  ...
  },
  {
    ...
    schema: { text: z.string(), delimiter: z.string().optional(), header: z.boolean().default(true) },
    handler: async (args: any) => {
      const { text, delimiter = ',', header = true } = args as { text: string; delimiter?: string; header?: boolean };
      ...
      if (header) {
        ...
        out = rows.slice(1).map((r: string[]) => Object.fromEntries(headers.map((h: string, i: number) => [h, (r as any)[i] ?? ''])));
      } else {
        out = rows;
      }
  ...
  },
  {
    ...
    schema: { value: z.number(), from: z.enum(['sqft', 'sqm']), to: z.enum(['sqft', 'sqm']) },
    handler: async (args: any) => {
      const { value, from, to } = args as { value: number; from: 'sqft' | 'sqm'; to: 'sqft' | 'sqm' };
      if (from === to) return { content: [{ type: 'text', text: JSON.stringify({ value }) }] };
      const sqmPerSqft = 0.09290304;
      const out = from === 'sqft' ? value * sqmPerSqft : value / sqmPerSqft;
      return { content: [{ type: 'text', text: JSON.stringify({ value: out }) }] };
    },
  },
  {
    name: 'util.units.convertDistance',
    description: 'Convert distance between miles and kilometers.',
    schema: { value: z.number(), from: z.enum(['mi', 'km']), to: z.enum(['mi', 'km']) },
    handler: async (args: any) => {
      const { value, from, to } = args as { value: number; from: 'mi' | 'km'; to: 'mi' | 'km' };
      if (from === to) return { content: [{ type: 'text', text: JSON.stringify({ value }) }] };
  ...
  },
  {
    ...
    handler: async (args: any) => {
      ...
      const re = /^(?<line1>.+?),\s*(?<city>[A-Za-z\s]+?),\s*(?<state>[A-Za-z]{2})\s*(?<zip>\d{5})(?:-\d{4})?$/;
      const m = t.match(re);
      if (!m || !m.groups) return { content: [{ type: 'text', text: JSON.stringify({ raw: t }) }] };
      return { content: [{ type: 'text', text: JSON.stringify({ line1: m.groups.line1, city: m.groups.city.trim(), state: m.groups.state.toUpperCase(), zip: m.groups.zip }) }] };
    },
  },
  {
    name: 'util.geo.distance',
    description: 'Haversine distance in km and mi between two coords.',
    schema: { lat1: z.number(), lng1: z.number(), lat2: z.number(), lng2: z.number() },
    handler: async (args: any) => {
      const { lat1, lng1, lat2, lng2 } = args as { lat1: number; lng1: number; lat2: number; lng2: number };
      const R = 6371; // km
  ...
  },
  {
    ...
    schema: { points: z.array(z.object({ lat: z.number(), lng: z.number() })).min(1) },
    handler: async (args: any) => {
      const { points } = args as { points: Array<{ lat: number; lng: number }> };
      const { sumLat, sumLng } = points.reduce((acc: any, p: any) => ({ sumLat: acc.sumLat + p.lat, sumLng: acc.sumLng + p.lng }), { sumLat: 0, sumLng: 0 });
      const lat = sumLat / points.length;
      const lng = sumLng / points.length;
      return { content: [{ type: 'text', text: JSON.stringify({ lat, lng }) }] };
    },
  },
];
