import { z } from 'zod';
import { bearer, httpDelete, httpGet, httpPost, httpPut } from '../core/http.js';
...
export const commuteTools: ToolDef[] = [
  {
    ...
    handler: async (args: any) => {
      ...
      const data = await httpPost('/api/commute-profiles', { name, destinations, maxMinutes, combine }, { headers: bearer(token) });
  ...
  },
  {
    ...
    handler: async (args: any) => {
      ...
      const data = await httpGet('/api/commute-profiles', { headers: bearer(token) });
      return { content: [{ type: 'text', text: JSON.stringify(data) }] };
    },
  },
  {
    ...
    handler: async (args: any) => {
      ...
      const data = await httpGet(`/api/commute-profiles/${encodeURIComponent(id)}`, { headers: bearer(token) });
      return { content: [{ type: 'text', text: JSON.stringify(data) }] };
    },
  },
  {
    ...
    handler: async (args: any) => {
      ...
      const data = await httpPut(`/api/commute-profiles/${encodeURIComponent(id)}`, body, { headers: bearer(token) });
      return { content: [{ type: 'text', text: JSON.stringify(data) }] };
    },
  },
  {
    ...
    handler: async (args: any) => {
      const { token, id } = args as { token: string; id: string };
      const data = await httpDelete(`/api/commute-profiles/${encodeURIComponent(id)}`, { headers: bearer(token) });
      return { content: [{ type: 'text', text: JSON.stringify(data) }] };
    },
  },
];
