...
export const marketTools: ToolDef[] = [
  {
    ...
    handler: async (args: any) => {
      ...
      const properties = data.results.filter((p: any) => p.price && p.livingArea);
      ...
      const pricesPerSqft = properties.map((p: any) => p.price / p.livingArea).sort((a: number, b: number) => a - b);
      const prices = properties.map((p: any) => p.price).sort((a: number, b: number) => a - b);
      ...
      const trends = {
        ...
        priceStats: {
          ...
          mean: Math.round(prices.reduce((sum: number, p: number) => sum + p, 0) / prices.length),
        ...
        pricePerSqftStats: {
          ...
          median: Math.round(pricesPerSqft[Math.floor(pricesPerSqft.length / 2)]),
          mean: Math.round(pricesPerSqft.reduce((sum: number, p: number) => sum + p, 0) / pricesPerSqft.length),
          q1: Math.round(pricesPerSqft[Math.floor(pricesPerSqft.length * 0.25)]),
          q3: Math.round(pricesPerSqft[Math.floor(pricesPerSqft.length * 0.75)]),
        },
        priceRanges: {
          under300k: properties.filter((p: any) => p.price < 300000).length,
          "300k-500k": properties.filter((p: any) => p.price >= 300000 && p.price < 500000).length,
          "500k-750k": properties.filter((p: any) => p.price >= 500000 && p.price < 750000).length,
          "750k-1m": properties.filter((p: any) => p.price >= 750000 && p.price < 1000000).length,
  ...
  },
  {
    ...
    handler: async (args: any) => {
      ...
      const targetData = await httpGet(`/api/properties/by-ids${qs({ ids: String(zpid) })}`);
      ...
      const similarData = await httpGet(`/api/graph/similar/${zpid}${qs({ limit: 20 })}`);
      ...
      if (!Array.isArray(similarData)) {
        return {
          content: [{
            ...
            text: JSON.stringify({
              target,
              error: "Similar properties not available (graph service may not be configured)",
            }),
          }],
        };
      }
      
      const comparables = similarData.filter((p: any) => p.price && p.livingArea);
      ...
      const analysis = {
        target: {
          ...
          pricePerSqft: target.price && target.livingArea ? Math.round(target.price / target.livingArea) : null,
        },
        market: {
          comparableCount: comparables.length,
          avgPrice: Math.round(
            comparables.reduce((sum: number, p: any) => sum + p.price, 0) / comparables.length,
          ),
          avgPricePerSqft: Math.round(
            comparables.reduce((sum: number, p: any) => sum + (p.price / p.livingArea), 0) / comparables.length,
          ),
          priceRange: {
        ...
        positioning: target.price && comparables.length > 0 ? {
          percentile: Math.round(
            (comparables.filter((p: any) => p.price < target.price).length / comparables.length) * 100,
          ),
          priceVsAvg: target.price - Math.round(
            comparables.reduce((sum: number, p: any) => sum + p.price, 0) / comparables.length,
          ),
          recommendation: (() => {
            const targetPsf = target.price / target.livingArea;
            const avgPsf = comparables.reduce((sum: number, p: any) => sum + (p.price / p.livingArea), 0) / comparables.length;
  ...
  },
  {
    ...
    handler: async (args: any) => {
      const { q, medianIncome = 80000, topK = 100 } = args as {
        q: string;
        medianIncome?: number;
        topK?: number;
      };
      ...
      const downPayment = medianPrice * 0.20;
      ...
      const affordability = {
        ...
        affordability: {
          ...
          affordableInventory: properties.filter((p: any) => p.price <= maxAffordable).length,
          affordablePercentage: Math.round(
            (properties.filter((p: any) => p.price <= maxAffordable).length / properties.length) * 100,
          ),
        },
        monthlyPaymentEstimate: {
          ...
          total: Math.round(monthlyPayment + (medianPrice * 0.011 / 12) + (medianPrice * 0.0035 / 12)),
          debtToIncomeRatio: `${dti.toFixed(1)}%`,
        },
        priceRanges: {
          affordable: properties.filter((p: any) => p.price <= maxAffordable).length,
          stretch: properties.filter((p: any) => p.price > maxAffordable && p.price <= maxAffordable * 1.2).length,
          outOfReach: properties.filter((p: any) => p.price > maxAffordable * 1.2).length,
        },
      };
      
      return { content: [{ type: "text", text: JSON.stringify(affordability, null, 2) }] };
    },
  },
];

/** Calculate monthly mortgage payment */
function calculateMonthlyPayment(principal: number, aprPercent: number, years: number): number {
