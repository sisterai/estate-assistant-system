...
function renderContentBlock(block: ContentBlock, opts?: { parseJsonText?: boolean }): string {
...
async function main() {
  ...
  const distDir = path.basename(__dirname) === 'dist' ? __dirname : path.join(__dirname, '..', 'dist');
  ...
  if (cmd === 'call') {
    ...
    if (!name) throw new Error('Usage: node dist/client.js call <toolName> <jsonArgs>');
  ...
  try {
    ...
    if ('content' in result && Array.isArray(result.content)) {
      for (const c of result.content) console.log(renderContentBlock(c, { parseJsonText: true }));
    } else {
      console.log(JSON.stringify(result, null, 2));
    }
  } catch (err) {
